...
class AppointmentForm extends StatelessWidget {
  const AppointmentForm({super.key, required this.expertName, });
        ...
        children: [
          Row(
            children: [
              ...
              GestureDetector(
                onTap: ()=> Get.back(),
                  child: const Icon(Iconsax.arrow_left_3,size: 22, color: TColors.primary)),
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
             Row(
              children: [
                const Text("Expert Name:",style: TextStyle(fontSize: 15),),
                const SizedBox(width: 5,),
                SizedBox(
                  width:110 ,
                    child: Text(expertName.toString(),style: const TextStyle(fontSize: 15,fontWeight: FontWeight.bold),maxLines: 1,overflow: TextOverflow.ellipsis,),),
            ...
            TextFormField(
              ...
              initialValue: currentUser.user.value.fullName,  // Set the initial value
            ...
            TextFormField(
              ...
              initialValue: currentUser.user.value.email,  // Set the initial value
              enabled: false, // Make the field read-only
            ),
            const SizedBox(height: TSize.spaceBtwInputFields),
               ...
               final date = controller.selectedDate.value;
                ...
                child: Row(
                  children: [
                    ...
                    Text(" Select Date: ${DateFormat('MM-dd-yyy').format(date)}",style: Theme.of(context).textTheme.bodyLarge,)
                  ],
                ),
              );
              }
            ...
            Obx((){
              ...
              return GestureDetector(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.access_time,color: TColors.primary,),
                    const SizedBox(width: 10),
                    Text("Select Time: ${time.format(context)}",style: Theme.of(context).textTheme.bodyLarge,),
...
void showAppointmentDialog (BuildContext context, String name) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return  AppointmentForm(expertName: name,); // Use the OtpDialog class
    },
  );
}
